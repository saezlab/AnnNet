...
def _write_structure(graph, path: Path, compression: str):
    ...
    def dict_to_parquet(d: dict, filepath: Path, id_name: str, val_name: str):
        ...
        df = pl.DataFrame({
            id_name: list(d.keys()),
            val_name: list(d.values())
        })
        df.write_parquet(filepath, compression=compression)
    
    dict_to_parquet(graph.entity_to_idx, path / "entity_to_idx.parquet", 
                    "entity_id", "idx")
    dict_to_parquet(graph.idx_to_entity, path / "idx_to_entity.parquet",
                    "idx", "entity_id")
    dict_to_parquet(graph.entity_types, path / "entity_types.parquet",
                    "entity_id", "type")
    dict_to_parquet(graph.edge_to_idx, path / "edge_to_idx.parquet",
                    "edge_id", "idx")
    dict_to_parquet(graph.idx_to_edge, path / "idx_to_edge.parquet",
                    "idx", "edge_id")
    dict_to_parquet(graph.edge_weights, path / "edge_weights.parquet",
                    "edge_id", "weight")
    dict_to_parquet(graph.edge_directed, path / "edge_directed.parquet",
                    "edge_id", "directed")
    dict_to_parquet(graph.edge_kind, path / "edge_kind.parquet",
                    "edge_id", "kind")
    ...
    edge_def_df.write_parquet(path / "edge_definitions.parquet", 
                              compression=compression)
    ...
    if graph.hyperedge_definitions:
        ...
        hyper_df = (
            pl.DataFrame({"edge_id": eids, "directed": dirs})
            .with_columns(
...
def _write_tables(graph, path: Path, compression: str):
    ...
    graph.vertex_attributes.write_parquet(
        path / "vertex_attributes.parquet", 
        compression=compression
    )
    graph.edge_attributes.write_parquet(
        path / "edge_attributes.parquet",
        compression=compression
    )
    graph.layer_attributes.write_parquet(
        path / "layer_attributes.parquet",
        compression=compression
    )
    graph.edge_layer_attributes.write_parquet(
        path / "edge_layer_attributes.parquet",
        compression=compression
    )
...
def _write_layers(graph, path: Path, compression: str):
    ...
    for layer_id, layer_data in graph._layers.items():
        registry_data.append({
            "layer_id": layer_id,
            "attributes": json.dumps(layer_data.get("attributes", {}))
        })
    pl.DataFrame(registry_data).write_parquet(
        path / "registry.parquet", compression=compression
    )
...
def _load_structure(graph, path: Path, lazy: bool):
    ...
    graph.entity_to_idx = parquet_to_dict(path / "entity_to_idx.parquet", 
                                         "entity_id", "idx")
    graph.idx_to_entity = parquet_to_dict(path / "idx_to_entity.parquet",
                                         "idx", "entity_id")
    graph.entity_types = parquet_to_dict(path / "entity_types.parquet",
                                        "entity_id", "type")
    graph.edge_to_idx = parquet_to_dict(path / "edge_to_idx.parquet",
                                       "edge_id", "idx")
    graph.idx_to_edge = parquet_to_dict(path / "idx_to_edge.parquet",
                                       "idx", "edge_id")
    graph.edge_weights = parquet_to_dict(path / "edge_weights.parquet",
                                        "edge_id", "weight")
    graph.edge_directed = parquet_to_dict(path / "edge_directed.parquet",
                                         "edge_id", "directed")
    graph.edge_kind = parquet_to_dict(path / "edge_kind.parquet",
                                     "edge_id", "kind")
...
def _load_tables(graph, path: Path):
    ...
    graph.edge_layer_attributes = pl.read_parquet(
        path / "edge_layer_attributes.parquet"
    )
