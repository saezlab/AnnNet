"""
WARNING: Naming this file `csv.py` will SHADOW Python's stdlib `csv`.
Prefer placing it under a package, e.g. `graph_io/csv.py`, and import as:

    from graph_io import csv as graph_csv
...
# Try to import the user's IncidenceGraph at runtime; otherwise the caller must pass an instance.
try:
    # Typical local module name; adjust if your project differs.
    from .incidence_graph import IncidenceGraph  # type: ignore
except Exception:  # noqa: BLE001 - we want to be permissive here
    IncidenceGraph = None  # type: ignore
