...
from .networkx_adapter import to_nx, from_nx, from_nx_only
    ...
    def clean_dict(d: dict):
        for k in list(d.keys()):
            ...
            try:
                ...
                d[k] = str(v)
    clean_dict(G.graph)
    for _, data in G.nodes(data=True): clean_dict(data)
    for _, _, data in G.edges(data=True): clean_dict(data)
...
def _restore_types_graphml_inplace(G):
    ...
    def coerce(s):
        ...
        t = s.strip()
        low = t.lower()
        if low in _BOOL: return _BOOL[low]
        if (t.startswith("{") and t.endswith("}")) or (t.startswith("[") and t.endswith("]")):
            try: return json.loads(t)
            except Exception: return t
        if _NUM_RE.match(t):
            try:
                if "." not in t and "e" not in low: return int(t)
                return float(t)
            except Exception:
                return t
        return t
    def fix_dict(d: dict):
        for k in list(d.keys()):
            d[k] = coerce(d[k])
    fix_dict(G.graph)
    for _, data in G.nodes(data=True): fix_dict(data)
    for _, _, data in G.edges(data=True): fix_dict(data)
...
def to_graphml(graph, path, *, directed=True, hyperedge_mode="reify", public_only=False):
    ...
    """
    G, manifest = to_nx(graph, directed=directed, hyperedge_mode=hyperedge_mode, public_only=public_only)
