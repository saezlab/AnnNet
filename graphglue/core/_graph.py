...
class Graph(BaseGraph):
    ...
    def __init__(self, default_edge_type: EdgeType = EdgeType.DIRECTED, **kwargs) -> None:
        ...
        # State for versioning and change tracking
        self._state = _State()
    ...
    def export(self, fmt: str = "networkx", **kwargs) -> Any:
        ...
        Parameters:
            fmt (str): Name of the adapter to use, e.g., "networkx", "igraph", etc.
            **kwargs: Additional arguments passed to the adapter's export method.

        Returns:
            Any: The exported graph in the specified format.
        """
        adapter = _backend_manager.get_adapter(fmt)
        return adapter.export(self, **kwargs)

    def to_networkx(self, **kwargs) -> "nx.Graph":
        """
        Convert this graph to a NetworkX MultiGraph/MultiDiGraph.

        Returns:
            A NetworkX graph object with equivalent nodes/edges/attributes.
        """
        return self.export(fmt="networkx", **kwargs)
