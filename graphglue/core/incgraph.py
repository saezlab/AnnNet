...
class IncidenceGraph:
    ...
    def subgraph_from_layer(self, layer_id, *, resolve_layer_weights=True):
        """Return a new IncidenceGraph restricted to one layer (Polars version)."""
        if layer_id not in self._layers:
            raise KeyError(f"Layer {layer_id} not found")
        ...
        for eid in layer_edges:
            ...
            w = self.get_effective_edge_weight(eid, layer=layer_id) if resolve_layer_weights else self.edge_weights[eid]
            ed_attrs = _row_attrs(self.edge_attributes, "edge_id", eid, "edge_id")
            edir = self.edge_directed.get(eid, self.directed)
            lg.add_edge(src, tgt, weight=float(w), edge_id=eid, edge_type=etype, edge_directed=edir, **ed_attrs)
    ...
    def copy(self):
        """Create a deep copy of the graph using Polars DataFrames."""
        ...
        for entity_id, entity_type in self.entity_types.items():
            if entity_type == "node":
                node_attrs = {}
                if (
                    isinstance(self.node_attributes, pl.DataFrame)
                    and self.node_attributes.height > 0
                    and "node_id" in self.node_attributes.columns
                ):
                    rows = self.node_attributes.filter(pl.col("node_id") == entity_id)
                    if rows.height > 0:
                        node_attrs = rows.to_dicts()[0]
        ...
        for edge_id, (source, target, edge_type) in self.edge_definitions.items():
            ...
            if (
                ...
                rows = self.edge_attributes.filter(pl.col("edge_id") == edge_id)
                if rows.height > 0:
                    edge_attrs = rows.to_dicts()[0]
                    edge_attrs.pop("edge_id", None)
        ...
        for eid, meta in self.hyperedge_definitions.items():
            ...
            if (
                ...
                rows = self.edge_attributes.filter(pl.col("edge_id") == eid)
                if rows.height > 0:
                    ed_attrs = rows.to_dicts()[0]
                    ed_attrs.pop("edge_id", None)
            if meta["directed"]:
        ...
        # Deep copy polars DataFrames
        new_graph.layer_attributes = self.layer_attributes.clone()
