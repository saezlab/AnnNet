...
def nx_backend(G: Graph, *,
    ...
    if hasattr(G.nx, "backend"):
        return G.nx.backend(directed=directed, hyperedge_mode=hyperedge_mode,
    ...
    return G.nx._get_or_make_nx(directed=directed, hyperedge_mode=hyperedge_mode,
                                layer=layer, layers=layers, needed_attrs=needed_attrs,
                                simple=simple, edge_aggs=edge_aggs)
...
def build_multi_graph() -> Graph:
    r"""
    MULTI-friendly graph for shortest paths etc.
    ...
    for v, name in [("a","alpha"),("b","bravo"),("c","charlie"),
                    ("d","delta"),("e","echo"),("f","phi"),
                    ("w","w"),("x","x"),("y","y"),("z","z")]:
...
def build_simple_mst_graph() -> Graph:
    r"""
    SIMPLE undirected weighted square + diagonals:
      A--1--B
      |\   /|
     2 \ /3 4
      | / \ |
...
class TestLazyNXProxy_MULTI(unittest.TestCase):
    ...
    def test_weighted_and_unweighted_shortest_paths(self):
        ...
        dist_w = G.nx.shortest_path_length(G, source="alpha", target="phi",
                                           weight="weight", _nx_label_field="name")
...
class TestLazyNXProxy_SIMPLE(unittest.TestCase):
    ...
    def test_max_flow_value(self):
        ...
        val = G.nx.maximum_flow_value(
            ...
            _nx_simple=True, _nx_edge_aggs={"capacity": "sum"}
        )
    ...
    def test_simple_collapse_aggregations(self):
        ...
        nxG = G.nx.backend(directed=False, simple=True,
